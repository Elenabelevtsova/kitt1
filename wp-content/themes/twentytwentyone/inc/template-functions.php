...
function twenty_twenty_one_continue_reading_text() {
	$continue_reading = sprintf(
		...
		esc_html__( 'Continue reading %s', 'kitone' ),
...
if ( ! function_exists( 'twenty_twenty_one_post_title' ) ) {
	...
	function twenty_twenty_one_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'kitone' ) : $title;
	}
}
 ...
 *
 * @since Kit-One 1.0
 * @since Kit-One 1.4 Corrected parameter name for `$output`,
...
function twenty_twenty_one_password_form( $output, $post = 0 ) {
	...
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'kitone' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'kitone' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'kitone' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'kitone' ) . '" /></form>
	';
	return $output;
}
